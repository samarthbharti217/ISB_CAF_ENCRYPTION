    ...
    def openFile1(self):
        ...
        if(v1.get()==1):
            encFile1 = pandas.read_csv(filepath1, dtype={'branch': str, 'account': str, 'daily_balance': str, 'date': str,
                                                       ...
                                                       'AADHAAR_linked': str},low_memory=False)

        elif(v1.get()==2):
            encFile1=pandas.read_csv(filepath1, dtype={'accno': str, 'account_type': str, 'CIF': str,
                                                       'customer_name': str, 'limit': str, 'interest_rate': str,
                                                       'theo_balance': str, 'outstanding': str, 'irregularity': str,'emi_due': str,
                                                       'emi_paid': str, 'emi_overdue': str, 'new_irac': str,'arrear_cond': str, 'currency': str,
                                                       'acct_maintain_branch': str,'emi_installment_amt': str, 'unrealised_int': str, 'accru_int': str,'file_date': str,
                                                       'branch': str, 'sanction_date': str, 'irregularity_date': str,'emi_installment': str},low_memory=False)
...
class PerfEncryp(tk.Frame):
    ...
    def start_now(self):
        ...
        self.pubVar
        public_key=None
        print(self.pubVar.get())
        print(self.pubVar.get())
        if(self.pubVar.get()==1):
                ...
                for i in range(len(names1)):
                    if box_vars1[i].get() == 1:
                        for index, row in encFile1.iterrows():
                            pres=names1[i]
                            encFile1[pres].values[index] = public_key.encrypt(str(encFile1[pres].values[index]).encode("utf8"),32)
                            count+=1
                            print(count)
            if v1.get() == 2:
                for i in range(len(names1)):
                    if box_vars1[i].get() == 1:
                        for index, row in encFile1.iterrows():
                            ...
                            print(count)
            filename1=name+".csv"
            encFile1.to_csv(os.path.join(self.savePath, filename1), encoding='utf-8',index=False)
      
        if(self.pubVar.get()==2):
    ...
    def publickeyfunc(self):
        self.selectPublic.config(state='normal')
        
    def dis(self):
        self.selectPublic.config(state='disabled')
    
    def getpubkey(self):
        global publicKey
        keyFile = filedialog.askopenfilename()
        self.public.config(text=keyFile)
        self.changeLocation3()
        key=open(keyFile,"r")
        private=key.read()
        publicKey=RSA.importKey(private)
        
    def changeLocation3(self):
    ...
    def __init__(self,parent,controller):
        ...
        button = tk.Button(self, text="Back",
                           command=lambda: controller.show_frame("EncryptPage"))
        self.public=Entry(self)
        
        self.public.pack(side="top")
        self.selectPublic=tk.Button(self, text="Select Public Key",state=DISABLED,command=self.getpubkey)
        self.selectPublic.pack(side="top")
        
        self.pubVar=IntVar()
        Radiobutton(self, text="Have existing public key", variable=self.pubVar, value=1,command=self.publickeyfunc).pack(anchor=W)
        Radiobutton(self, text="Generate new public/private key pair", variable=self.pubVar, value=2,command=self.dis).pack(anchor=W)
        ...
        if (v2.get() == 1):
            ...
            encFile2 = pandas.read_csv(self.filepath2,
                                      dtype={'accno': str, 'account_type': str, 'CIF': str, 'customer_name': str,
                                             ...
                                             'irregularity_date': str, 'emi_installment': str},low_memory=False)
...
class PerfDecryp(tk.Frame):
    def start_now2(self):
            ...
            for i in range(len(names2)):
                ...
                if box_vars2[i].get() == 1:
                    for index, row in encFile2.iterrows():
                        pres = names2[i]
                        x = privateKey.decrypt(ast.literal_eval(str(encFile2[pres].values[index])))
                        encFile2[pres].values[index]=x.decode('utf-8')
        if v2.get() == 2:
            for i in range(len(names2)):
        ...
        filename2 = name + ".csv"
        encFile2.to_csv(os.path.join(self.savePath, filename2), encoding='utf-8',index=False)
...
if __name__ == "__main__":
    if(sys.argv[1]=="CL"):
        enc_or_dec=sys.argv[2]              #Encrypt=1 ; Decrypt=2
        loan_or_savings=sys.argv[3]         #Loan=1 ; Savings=2
        file_location=sys.argv[4]           #Location of the file to be encrypted
        column_no=int(sys.argv[5])          #Total number of columns that need to be encrypted
        existing_key=sys.argv[6]            #1=Existing key exists ; 2 = Existing key does not exist 
        existing_key_location=sys.argv[7]   #Location of existing key ; Enter None if no key
        save_location=sys.argv[8]           #location of the folder where key has to be saved
        save_file_name=sys.argv[9]          #Name of the file to be saved as
        column=[]
        for i in range(column_no):
            column.append(int(input("Enter "+str(i)+"th column index: ").strip()))
        if(loan_or_savings=="2"):
            enc = pandas.read_csv(file_location,
                                  dtype={'branch': str, 'account': str, 'daily_balance': str, 'date': str,
                                         'branch_cluster': str, 'pmjdy': str, 'date1': str, 'date2': str,
                                         'description': str, 'debit_credit': str, 'amount': str,
                                         'debit_credit1': str, 'balance': str, 'journal_no': str,
            ...
            if(enc_or_dec=="1"):
                if(existing_key=="0"):
                    ...
                    key = RSA.generate(1024, random_generator)
                    public_key = key.publickey()
                    PublicKeyFile=("PublicKey_"+save_file_name)
                    f = open(os.path.join(save_location, PublicKeyFile), "w")
                    f.write((key.publickey().exportKey('PEM')).decode("utf8"))
                    f.close()
                    PrivateKeyFile=("PrivateKey_"+save_file_name)
                    f = open(os.path.join(save_location, PrivateKeyFile), "w")
                    f.write((key.exportKey('PEM')).decode("utf8"))
                    f.close()
                    for i in column:
                        pres=enc.columns[i]
                        for index, row in enc.iterrows():
                            print(1)
                            enc[pres].values[index] = public_key.encrypt(str(enc[pres].values[index]).encode("utf8"),32)
                
                elif(existing_key=="1"):
                    key=open(existing_key[2],"r")
                    public=key.read()
                ...
                enc.to_csv(os.path.join(save_location, save_file_name), encoding='utf-8',index=False)
                print(os.path.join(save_location, save_file_name))
                            
            elif(enc_or_dec=="2"):
                key=open(existing_key_location,"r")
                private=key.read()
                privateKey=RSA.importKey(private)
                for i in column:
                    pres=enc.columns[i]
                    for index, row in enc.iterrows():
                        pres=enc.columns[int(column_no[i])]
                        x = privateKey.decrypt(ast.literal_eval(str(enc[pres].values[index])))
                        enc[pres].values[index] = x.decode('utf-8')
                
                enc.to_csv(os.path.join(save_location, save_file_name), encoding='utf-8',index=False)
                
        elif(loan_or_savings=="1"):
            print("Inside Savings")
            enc = pandas.read_csv(file_location,dtype={'accno': str, 'account_type': str, 'CIF': str, 'customer_name': str,
                                         'limit': str, 'interest_rate': str, 'theo_balance': str,
                                         'outstanding': str, 'irregularity': str, 'emi_due': str,
                                         'emi_paid': str, 'emi_overdue': str, 'new_irac': str,
                                         'arrear_cond': str, 'currency': str, 'acct_maintain_branch': str,
